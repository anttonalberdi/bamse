...
if mode == 'ligation':
    trim_ligation1 = 'cutadapt --pair-adapters -e 0.2 -g ^'+primer1+' -G ^'+primer2+' --discard-untrimmed -o '+output1+'_a -p '+output2+'_a '+input1+' '+input2+''
    subprocess.Popen(trim_ligation1, shell=True).wait()
    trim_ligation2 = 'cutadapt --pair-adapters -e 0.2 -g ^'+primer2+' -G ^'+primer1+' --discard-untrimmed -o '+output1+'_b -p '+output2+'_b '+input1+' '+input2+''
    ...
    trim_PCR = 'cutadapt -e 0.5 -g ^'+primer1+' -G ^'+primer2+' --discard-untrimmed -o '+output1+' -p '+output2+' '+input1+' '+input2+''
